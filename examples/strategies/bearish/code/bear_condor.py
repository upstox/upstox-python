...
Buys an ATM put, sells ATM-1 and ATM-2 puts, and buys an ATM-3 put
for the next weekly expiry. Profits when Nifty falls moderately into
the middle zone between the two short strikes.
"""
...
try:
    ...
    print(f"Upper short (ATM-1) - Trading symbol : {upper_short['trading_symbol']}")

    lower_short = instruments_api.search_instrument(
        "Nifty 50", exchanges="NSE", segments="FO",
        instrument_types="PE", expiry="next_week", atm_offset=-2
    ).data[0]
    print(f"Lower short (ATM-2) - Trading symbol : {lower_short['trading_symbol']}")

    lower_long = instruments_api.search_instrument(
        "Nifty 50", exchanges="NSE", segments="FO",
        instrument_types="PE", expiry="next_week", atm_offset=-3
    ).data[0]
    print(f"Lower long  (ATM-3) - Trading symbol : {lower_long['trading_symbol']}")

    # Step 2: Buy the upper long put (ATM)
    ...
    # Step 3: Sell the upper short put (ATM-1)
    ...
    print(f"Upper short put order placed successfully. Order ID: {response2}")

    # Step 4: Sell the lower short put (ATM-2)
    ...
    print(f"Lower short put order placed successfully. Order ID: {response3}")

    # Step 5: Buy the lower long put (ATM-3)
    ...
    print(f"Lower long put order placed successfully. Order ID: {response4}")
