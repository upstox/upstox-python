...
Sells one ATM call and buys two OTM calls (ATM+1) for the next weekly expiry.
Profits from a large upward move. The sold ATM call finances the two bought
OTM calls, making this a low-cost or net-credit entry.
"""
...
try:
    ...
    print(f"Short call (ATM)   - Trading symbol : {short_call['trading_symbol']}")
    print(f"Short call (ATM)   - Instrument key : {short_call['instrument_key']}")

    # Step 2: Find the OTM call (long leg — 2x quantity)
