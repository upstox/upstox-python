...
Sells an OTM Nifty 50 call (ATM+1) and an OTM put (ATM-1) for the next weekly expiry.
"""
...
try:
    ...
    # Step 1: Find the OTM call option one strike above ATM
    ...
    print(f"OTM call - Trading symbol : {otm_call['trading_symbol']}")
    print(f"OTM call - Instrument key : {otm_call['instrument_key']}")

    # Step 2: Find the OTM put option one strike below ATM
    otm_put = instruments_api.search_instrument(
        "Nifty 50", exchanges="NSE", segments="FO",
        instrument_types="PE", expiry="next_week", atm_offset=-1
    ).data[0]
    print(f"OTM put  - Trading symbol : {otm_put['trading_symbol']}")
    print(f"OTM put  - Instrument key : {otm_put['instrument_key']}")

    # Step 3: Sell the OTM call
