"""
PCR Data — Put-Call Ratio for an underlying + expiry with intraday insights.

Fetches overall PCR plus intraday PCR / spot data points at the requested
bucket interval.

Usage:
  python market_information/pcr_data.py --token <TOKEN> --expiry 2026-05-29
  python market_information/pcr_data.py --token <TOKEN> --instrument-key "NSE_INDEX|Nifty Bank" --expiry 2026-05-29 --bucket-interval 15
"""
...
def _fmt_pcr(v):
    ...
    try:
        f = float(v)
        color = RED if f < 0.7 else (GREEN if f > 1.3 else CYAN)
        return f"{color}{f:>8.3f}{RESET}"
    except (TypeError, ValueError):
        return str(v)


def main():
    parser = argparse.ArgumentParser(description="PCR via Market API")
    ...
    print(f"\nFetching PCR for {args.instrument_key} expiry={args.expiry} "
    ...
    pcr      = data.get("pcr") or data.get("put_call_ratio")
    spot     = data.get("spot_closing_price")
    insights = data.get("insights") or []

    print(f"  {CYAN}Overall PCR  {RESET} {_fmt_pcr(pcr)}")
    ...
    col_w = 12
    print(f"  {BOLD}{'Timestamp':<24} {'PCR':>{col_w}} {'Spot':>{col_w}}{RESET}")
    print("  " + "─" * (24 + (col_w + 1) * 2))

    for ins in insights:
        ins = _as_dict(ins)
        ts  = ins.get("timestamp") or ins.get("time") or "—"
        p   = ins.get("pcr") or ins.get("put_call_ratio")
        sp  = ins.get("spot_price")
        print(f"  {str(ts):<24} {_fmt_pcr(p):>{col_w + 9}} {_fmt_num(sp):>{col_w}}")
